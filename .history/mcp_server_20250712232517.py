...
async def comprehensive_project_indexing(
    ...
    """
    🚀 통합 프로젝트 인덱싱 도구 - 파일 스캔 + 인덱싱 + 벡터DB 저장 + 컨텍스트 분석
    
    이 도구는 Docker 볼륨 마운트된 프로젝트의 모든 파일을 스캔하고, 내용을 분석하여 
    벡터 데이터베이스에 저장하는 전체 과정을 하나의 통합된 워크플로우로 실행합니다.
    
    Args:
        project_path: 인덱싱할 프로젝트 경로 (예: /host_projects/lovechedule-app)
        project_id: 프로젝트 식별자 (기본값: "default")
    ...
    Note:
        - Docker 볼륨 마운트 환경에서 사용
        - 파일 업로드는 network_project_upload() 함수 사용
        - AutoIndexingService가 백그라운드에서 자동 처리
    """
...
async def stop_auto_indexing() -> Dict[str, Any]:
    ...
    try:
        ...
        if not auto_indexing_service.is_running:
            return {
                "success": True,
                "message": "자동 인덱싱 서비스가 이미 중지되어 있습니다"
            }
