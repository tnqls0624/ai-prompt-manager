...
@handle_errors(
    category=ErrorCategory.SYSTEM,
    level=ErrorLevel.LOW,
    return_on_error={"error": "자동 인덱싱 상태 조회 실패", "success": False}
)
async def get_auto_indexing_status() -> Dict[str, Any]:
    """
    자동 백그라운드 인덱싱 서비스 상태 조회
    ...
    try:
        ...
        if not auto_indexing_service:
            return {
                "success": False,
                "error": "자동 인덱싱 서비스가 초기화되지 않았습니다"
            }
        
        status = await auto_indexing_service.get_status()
        return {
            "success": True,
            "status": status,
            "message": "자동 인덱싱 상태 조회 성공"
        }
        
    except Exception as e:
        logger.error(f"자동 인덱싱 상태 조회 중 오류: {e}")
        return {
            "success": False,
            "error": str(e)
        }
...
@handle_errors(
    category=ErrorCategory.SYSTEM,
    level=ErrorLevel.MEDIUM,
    user_message="강제 재스캔 중 오류가 발생했습니다."
)
async def force_auto_rescan(project_name: str = None) -> Dict[str, Any]:
    """
    자동 인덱싱 서비스 강제 재스캔 실행
    
    Args:
        project_name: 특정 프로젝트만 재스캔 (없으면 전체)
    ...
    try:
        ...
        if not auto_indexing_service.is_running:
            return {
                "success": False,
                "error": "자동 인덱싱 서비스가 실행 중이지 않습니다"
            }
        
        # 백그라운드에서 재스캔 실행
        asyncio.create_task(auto_indexing_service.force_rescan(project_name))
        
        return {
            "success": True,
            "message": f"강제 재스캔이 시작되었습니다 (프로젝트: {project_name or '전체'})",
            "project_name": project_name
        }
        
    except Exception as e:
        logger.error(f"강제 재스캔 중 오류: {e}")
        return {
            "success": False,
            "error": str(e)
        }
...
@handle_errors(
    category=ErrorCategory.SYSTEM,
    level=ErrorLevel.MEDIUM,
    user_message="자동 인덱싱 서비스 시작 중 오류가 발생했습니다."
)
async def start_auto_indexing() -> Dict[str, Any]:
    """
    자동 인덱싱 서비스 시작
    ...
    try:
        ...
        if auto_indexing_service.is_running:
            return {
                "success": True,
                "message": "자동 인덱싱 서비스가 이미 실행 중입니다"
            }
...
@handle_errors(
    category=ErrorCategory.SYSTEM,
    level=ErrorLevel.MEDIUM,
    user_message="자동 인덱싱 서비스 중지 중 오류가 발생했습니다."
)
async def stop_auto_indexing() -> Dict[str, Any]:
    """
    자동 인덱싱 서비스 중지
    ...
    try:
        ...
        logger.error(f"자동 인덱싱 서비스 중지 중 오류: {e}")
        return {
            "success": False,
            "error": f"자동 인덱싱 서비스 중지 실패: {str(e)}"
        }
...
@handle_errors(
    category=ErrorCategory.SYSTEM,
    level=ErrorLevel.MEDIUM,
    user_message="사용 가능한 프로젝트 조회 중 오류가 발생했습니다."
)
@measure_performance(operation_name="get_available_projects", threshold=5.0)
async def get_available_projects() -> Dict[str, Any]:
    ...
    try:
        if auto_indexing_service:
            projects = await auto_indexing_service.get_available_projects()
        ...
        logger.error(f"사용 가능한 프로젝트 조회 중 오류: {e}")
        return {
            "success": False,
...
@handle_errors(
    category=ErrorCategory.SYSTEM,
    level=ErrorLevel.MEDIUM,
    user_message="선택된 프로젝트 조회 중 오류가 발생했습니다."
)
@measure_performance(operation_name="get_selected_projects", threshold=3.0)
async def get_selected_projects() -> Dict[str, Any]:
    """
    선택된 프로젝트 목록 조회
    ...
    try:
        if auto_indexing_service:
            selected_projects = await auto_indexing_service.get_selected_projects()
        ...
        logger.error(f"선택된 프로젝트 조회 중 오류: {e}")
        return {
            "success": False,
            "error": f"선택된 프로젝트 조회 실패: {str(e)}"
        }
...
@handle_errors(
    category=ErrorCategory.SYSTEM,
    level=ErrorLevel.MEDIUM,
    user_message="프로젝트 선택 중 오류가 발생했습니다."
)
@measure_performance(operation_name="select_project_for_indexing", threshold=5.0)
async def select_project_for_indexing(
    project_name: str,
    ...
    Args:
        project_name: 선택할 프로젝트 이름
        auto_indexing: 자동 인덱싱 활성화 여부
        file_watcher: 파일 감시 활성화 여부
        max_file_size: 최대 파일 크기 (바이트)
        max_files: 최대 파일 개수
        include_extensions: 포함할 파일 확장자 목록
        exclude_directories: 제외할 디렉토리 목록
    ...
    try:
        if auto_indexing_service:
            ...
            if include_extensions is None:
                include_extensions = ['.py', '.js', '.ts', '.jsx', '.tsx', '.html', '.css', '.md', '.json', '.yaml', '.yml']
            
            # 기본 제외 디렉토리 설정
            if exclude_directories is None:
                exclude_directories = ['node_modules', '.git', '__pycache__', 'dist', 'build', '.vscode', '.idea']
            ...
            # 프로젝트 선택
            success = await auto_indexing_service.add_project_to_whitelist(project_name, config)
            
            if success:
                return {
                    "success": True,
                    "message": f"프로젝트 '{project_name}'이(가) 선택되었습니다",
                    "project_name": project_name,
                    "config": config,
                    "timestamp": datetime.now().isoformat()
                }
                ...
                return {
                    "success": False,
                    "error": f"프로젝트 '{project_name}' 선택 실패"
                }
        else:
            return {
                "success": False,
        ...
        logger.error(f"프로젝트 선택 중 오류: {e}")
        return {
            "success": False,
            "error": f"프로젝트 선택 실패: {str(e)}"
        }
...
@handle_errors(
    category=ErrorCategory.SYSTEM,
    level=ErrorLevel.MEDIUM,
    user_message="프로젝트 선택 해제 중 오류가 발생했습니다."
)
@measure_performance(operation_name="deselect_project_from_indexing", threshold=3.0)
async def deselect_project_from_indexing(project_name: str) -> Dict[str, Any]:
    """
    프로젝트를 인덱싱 대상에서 제외
    
    Args:
        project_name: 제외할 프로젝트 이름
    ...
    try:
        if auto_indexing_service:
            success = await auto_indexing_service.remove_project_from_whitelist(project_name)
            
            if success:
                return {
                    "success": True,
                    "message": f"프로젝트 '{project_name}'이(가) 제외되었습니다",
                    "project_name": project_name,
                    "timestamp": datetime.now().isoformat()
                }
            else:
                return {
                    "success": False,
                    "error": f"프로젝트 '{project_name}' 제외 실패"
                }
        else:
            return {
                "success": False,
        ...
        logger.error(f"프로젝트 제외 중 오류: {e}")
        return {
            "success": False,
            "error": f"프로젝트 제외 실패: {str(e)}"
        }
...
@handle_errors(
    category=ErrorCategory.SYSTEM,
    level=ErrorLevel.MEDIUM,
    user_message="프로젝트 설정 업데이트 중 오류가 발생했습니다."
)
@measure_performance(operation_name="update_project_indexing_config", threshold=3.0)
async def update_project_indexing_config(
    project_name: str,
    auto_indexing: bool = None,
    file_watcher: bool = None,
    ...
    try:
        if auto_indexing_service:
            ...
            if auto_indexing is not None:
                config['auto_indexing'] = auto_indexing
            ...
            if include_extensions is not None:
                config['include_extensions'] = include_extensions
            if exclude_directories is not None:
                config['exclude_directories'] = exclude_directories
            
            success = await auto_indexing_service.update_project_config(project_name, config)
            
            if success:
                return {
                    "success": True,
                    "message": f"프로젝트 '{project_name}' 설정이 업데이트되었습니다",
                    "project_name": project_name,
                ...
                return {
                    "success": False,
                    "error": f"프로젝트 '{project_name}' 설정 업데이트 실패"
                }
        else:
            return {
                "success": False,
        ...
        logger.error(f"프로젝트 설정 업데이트 중 오류: {e}")
        return {
            "success": False,
            "error": f"프로젝트 설정 업데이트 실패: {str(e)}"
        }
...
@handle_errors(
    category=ErrorCategory.SYSTEM,
    level=ErrorLevel.MEDIUM,
    user_message="프로젝트 강제 인덱싱 중 오류가 발생했습니다."
)
@measure_performance(operation_name="force_index_project", threshold=30.0)
async def force_index_project(project_name: str) -> Dict[str, Any]:
    """
    특정 프로젝트 강제 인덱싱
    
    Args:
        project_name: 강제 인덱싱할 프로젝트 이름
    ...
    try:
        if auto_indexing_service:
            result = await auto_indexing_service.force_index_project(project_name)
            
            if result.get('success'):
                return {
                    "success": True,
                    "message": f"프로젝트 '{project_name}' 강제 인덱싱이 완료되었습니다",
                    "project_name": project_name,
                ...
                return {
                    "success": False,
                    "error": result.get('error', '강제 인덱싱 실패')
                }
        else:
            return {
                "success": False,
        ...
        logger.error(f"프로젝트 강제 인덱싱 중 오류: {e}")
        return {
            "success": False,
            "error": f"강제 인덱싱 실패: {str(e)}"
        }
...
@handle_errors(
    category=ErrorCategory.SYSTEM,
    level=ErrorLevel.LOW,
    return_on_error={"error": "선택적 인덱싱 상태 조회 실패", "success": False}
)
@measure_performance(operation_name="get_selective_indexing_status", threshold=3.0)
async def get_selective_indexing_status() -> Dict[str, Any]:
    """
    선택적 인덱싱 상태 조회
    ...
    try:
        if auto_indexing_service:
            ...
            return {
                "success": True,
                "selective_indexing_enabled": status.get('selective_indexing_enabled', False),
                "auto_indexing_enabled": status.get('auto_indexing_enabled', False),
                "selected_projects": status.get('selected_projects', []),
                "selected_projects_count": status.get('selected_projects_count', 0),
                ...
                "last_scan_time": status.get('last_scan_time', '없음'),
                "timestamp": datetime.now().isoformat()
            }
        else:
            return {
        ...
        logger.error(f"선택적 인덱싱 상태 조회 중 오류: {e}")
        return {
            "success": False,
...
async def cleanup_services():
    """서비스 정리"""
    global auto_indexing_service, file_watcher_service
    ...
    try:
        ...
        if file_watcher_service:
            await file_watcher_service.stop_all_watching()
            logger.info("파일 감시 서비스 중지 완료")
